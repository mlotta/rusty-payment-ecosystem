...
pub async fn get_balance(repo: &dyn BankRepository, uuid: Uuid) -> Result<Option<Customer>, InterfaceError>{
...
pub async fn create_account(repo: &dyn BankRepository, customer: &Customer) -> Result<(), InterfaceError>{
...
pub async fn authorize_transaction(repo: &dyn BankRepository, uuid: Uuid, amount: i32) -> Result<(), InterfaceError>{
    if amount <= 0 {
        return Err(InterfaceError::Other("amount of transaction needs to be positive".to_string()));
    }
    ...
    if amount > customer.unwrap().balance {
        return Err(InterfaceError::Other("transaction refused: not enough balance".to_string()))
    }
